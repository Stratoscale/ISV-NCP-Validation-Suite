#!/usr/bin/env python3
"""Tear down a zcompute bare-metal instance and associated resources.

Same as scripts/vm/teardown.py, with a longer termination poll timeout —
bare-metal termination can take 15-20+ minutes.
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if code in ("DependencyViolation", "InvalidGroup.InUse"):
                ...
                print(f"[teardown] failed to delete SG {sg_id}: {e}", file=sys.stderr)
                return False
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Tear down a zcompute bare-metal instance")
    ...
    parser.add_argument("--delete-key-pair", action="store_true", default=False)
    parser.add_argument("--delete-security-group", action="store_true", default=False)
    parser.add_argument("--skip-destroy", action="store_true", default=False)
    ...
    try:
        ...
        print(
            f"[teardown] instance {args.instance_id} state: {current_state}, key: {key_name}, SGs: {sg_ids}",
            file=sys.stderr,
        )
        ...
        if current_state not in ("terminated", "shutting-down"):
            ...
            print(f"[teardown] instance already in state '{current_state}'; skipping terminate", file=sys.stderr)

        # Bare-metal termination can take 15-20+ minutes.
        final_state = poll_instance_state(ec2, args.instance_id, ["terminated"], timeout=1800, interval=30)
        ...
        if args.eip_allocation_id:
            try:
                ...
                print(f"[teardown] EIP {args.eip_allocation_id} released", file=sys.stderr)
            except ClientError as e:
                code = e.response.get("Error", {}).get("Code", "")
                print(f"[teardown] failed to release EIP {args.eip_allocation_id}: {code} — {e}", file=sys.stderr)

        if args.delete_security_group and sg_ids:
            for sg_id in sg_ids:
                if _delete_security_group(ec2, sg_id):
        ...
        if args.delete_key_pair and key_name:
            try:
                ...
                print(f"[teardown] failed to delete key pair '{key_name}': {e}", file=sys.stderr)
